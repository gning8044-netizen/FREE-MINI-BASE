...
const inconnuboyConfigSchema = new mongoose.Schema({
    ...
    config: {
        ...
        AUTO_STATUS_MSG: { type: String, default: 'Hello from DEV SHADOW! 🔥' },
...
async function saveSessionToMongoDB(number, credentials) {
    try {
        ...
        console.log(`📁 [DEV SHADOW] Session saved for ${cleanNumber}`);
        return true;
    } catch (error) {
    ...
    try {
        ...
        if (config) {
            ...
            const defaultConfig = {
                ...
                AUTO_STATUS_MSG: 'Hello from DEV SHADOW! 🔥',
                AUTO_LIKE_EMOJI: ['❤️', '👍', '😮', '😎'],
                ANTIDELETE: 'false'
            };
...
async function updateUserConfigInMongoDB(number, newConfig) {
    try {
        ...
        console.log(`⚙️ [DEV SHADOW] Config updated for ${cleanNumber}`);
        return true;
    } catch (error) {
